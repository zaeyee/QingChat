<template>
	<view class="mescroll-body mescroll-render-touch" :class="{'mescorll-sticky': sticky}" :style="{'minHeight':minHeight, 'padding-top': padTop, 'padding-bottom': padBottom}"
		 ...
		 :change:prop="wxsBiz.callObserver" :prop="callProp">
			...
			<view v-if="mescroll.optDown.use" class="mescroll-downwarp" :style="{'background':mescroll.optDown.bgColor,'color':mescroll.optDown.textColor}">
				<view class="downwarp-content">
					<view v-show="downLoadType!==4" class="downwarp-progress mescroll-wxs-progress" :class="{'mescroll-rotate': this.isDownLoading}"
					 :style="progressStyle"></view>
					<view v-show="downLoadType===4" class="downwarp-progress" :class="mescroll.optDown.endFail ? 'result-fail' : 'result-success'"
					 :style="resultStyle"></view>
					<view class="downwarp-tip" :style="tipStyle">{{downText}}</view>
				</view>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				downLoadType: 0, // 下拉刷新状态: 0(loading前), 1(inOffset), 2(outOffset), 3(showLoading), 4(showEnding)
		...
		computed: {
			...
			// 是否为重置下拉的状态
			isDownReset() {
				return this.downLoadType === 3 || this.downLoadType === 0;
			},
			// 过渡
			transition() {
			...
			downText() {
				...
				switch (this.downLoadType) {
					...
					case 4:
						return this.mescroll.optDown.textEnding;
					default:
						return this.mescroll.optDown.textInOffset;
				}
			},
			// 进度条样式
			progressStyle() {
				let size = this.toPx(this.mescroll.optDown.textSize) + 2 + 'px'
				let styleObj = {
					'width': size,
					'height': size,
					'border-color': this.mescroll.optDown.textColor,
					'transform': 'rotate(' + 360 * this.downRate + 'deg)'
				}
				return styleObj
			},
			// 下拉结果样式
			resultStyle() {
				let size = this.toPx(this.mescroll.optDown.textSize) + 2 + 'px'
				let color = this.mescroll.optDown.textColor
				let styleObj = {
					'width': size,
					'height': size,
					'--color': color,
			...
			tipStyle() {
				return {
					'font-size': this.toPx(this.mescroll.optDown.textSize) + 'px'
				}
			}
		},
		methods: {
		...
		created() {
			...
			let diyOption = {
				// 下拉刷新的配置
				down: {
					textSize: '24', // 初始下拉文字大小，默认24(默认单位为rpx)
					textEnding: '', // 初始结束语文字，默认为空
					endFail: false, // 初始结束语状态，默认成功
					inOffset() {
					...
					beforeEndDownScroll() {
						// 如果设置结束语且正在显示下拉进度，则显示结束语
						if (vm.mescroll.optDown.textEnding && vm.isDownLoading) {
							vm.downLoadType = 4
							return this.endingTime ? this.endingTime : 400
						}
					},
					endDownScroll() {
